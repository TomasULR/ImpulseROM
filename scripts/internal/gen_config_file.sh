...
IS_IMPULSE_CERT_AVAILABLE()
{
    local USES_IMPULSE_CERT="false"
    [ -s "$SRC_DIR/security/impulse_platform.pk8" ] && USES_IMPULSE_CERT="true"
    echo "$USES_IMPULSE_CERT"
}
# ]
...

{
    ...
    echo "ROM_IS_OFFICIAL=\"$(IS_IMPULSE_CERT_AVAILABLE)\""
    ...
    GET_BUILD_VAR "MULTILIB_FIRMWARE" ""
    GET_BUILD_VAR "SOURCE_METADATA_AUTODETECT" "false"
    ...
    GET_BUILD_VAR "TARGET_SYSTEM_AS_ROOT" "true"
    GET_BUILD_VAR "TARGET_BOOT_DEVICE_PATH" "/dev/block/bootdevice/by-name"
    GET_BUILD_VAR "TARGET_SYSTEM_PARTITION_SIZE" "none"
    GET_BUILD_VAR "TARGET_PRISM_PARTITION_SIZE" "none"
    GET_BUILD_VAR "TARGET_OPTICS_PARTITION_SIZE" "none"
